...
Pipeline de ingesta near-real-time del feed GBFS de Ecobici CDMX (operado por
Lyft). Cada 10 minutos descarga el estado de las estaciones, lo limpia con
Polars y mantiene un único Parquet por día en MinIO.

Etapas:
    1. ensure_bronze_setup
       Crea el bucket, el schema y la tabla externa Hive particionada por
       `dt` en Trino. Operación idempotente (no falla si ya existen).

    2. fetch_station_status
       Descarga el JSON del feed GBFS `station_status`.

    3. clean_and_append
       Limpia y tipa el snapshot con Polars. Lee el Parquet del día (si
       existe), lo une con el snapshot nuevo, deduplica por
       (station_id, last_reported) y reescribe un único archivo en:
       'bck-bronze/ecobici/station_status/dt=YYYY-MM-DD/data.parquet'.

    4. sync_trino_partitions
       Ejecuta 'system.sync_partition_metadata' para que Trino descubra
       la partición del día. Necesario porque el conector Hive no detecta
       particiones nuevas automáticamente.

Notas de diseño:
    - Feed elegido: 'station_status' (estado en tiempo real de bicis/docks
      disponibles por estación), sobre otros feeds GBFS estáticos como
      'station_information'.
    - Cadencia de extracción: cada 10 minutos. El feed declara ttl=10s y se
      actualiza con altísima frecuencia, pero un polling cada 10 min ya
      produce ~144 snapshots/día — suficiente para análisis de ocupación
      hora-pico vs. valle, sin saturar la API ni inflar el lake.
    - Un único Parquet por partición (no micro-archivos por corrida): cada
      ejecución trata el archivo del día como una tabla pequeña en memoria,
      lo reescribe completo tras deduplicar. Mantiene idempotencia (correr
      el mismo snapshot dos veces no duplica filas) a costa de una
      rescritura más cara conforme avanza el día.
    - max_active_runs=1 para evitar condiciones de carrera entre corridas
      concurrentes sobre el mismo archivo del día.
"""
...
def ecobici_station_status_etl():
    ...
    def clean_and_append(payload: dict) -> str:
        ...
        try:
            ...
            print(f"WARN: no pude leer el parquet ({key}): {e}")
            existing = None
    ...
    setup_task = ensure_bronze_setup()
    payload_task = fetch_station_status()
    parquet_path_task = clean_and_append(payload_task)
    sync_task = sync_trino_partitions()

    setup_task >> payload_task >> parquet_path_task >> sync_task
